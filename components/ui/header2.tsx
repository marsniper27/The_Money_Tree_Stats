...
export default function Header() {
  ...
  const [connectedChainId, setChainId] = useState('');
  const [connectedChain, setChain] = useState('');
    ...
    try {
      ...
      if (provider) {
        ...
        if (web3) {
          ...
          if (isChainSupported) {
            setChain(supportedChains.id[connectedChainId]);
  ...
  const handleChainSelection = async (selectedChain: string) => {
    ...
    if (selectedChain) {
      try {
        const chainSet = await (window as any).ethereum.request({
